...
$sa_taxo_r_config = $this->get_config();
$selected_terms = $this->get_data($article_id, $_POST);
$multiple = $sa_taxo_r_config['field_config']['multiple'] ? 'multiple="multiple"':'';

?>

<div class="sa-field-front sa-field-content" style="width: calc(<?php echo $this->get_width();?> - 30px)">
    <span class="titlelabel"><?php echo $sa_taxo_r_config['field_config']['label']; ?></span>

    <script type="text/javascript">
        jQuery(function(){
            jQuery("#<?php echo $this->slug;?>").select2(
                {
                    placeholder:"<?php echo $sa_taxo_r_config['field_config']['placeholder']; ?>",
