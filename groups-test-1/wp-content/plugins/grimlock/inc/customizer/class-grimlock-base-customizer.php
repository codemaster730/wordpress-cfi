...
abstract class Grimlock_Base_Customizer {
	 ...
	 * @param string $theme_mod The name of the theme mod
	 *
	 * @return array The css vars for the Kirki typography field
	 */
	protected function get_typography_css_vars( $theme_mod ) {
		if ( empty( $theme_mod ) ) {
			return array();
		}
		...
		return array_map( function( $option ) use( $var_prefix ) {
			...
			return array( "--grimlock-{$var_prefix}-{$option}", $value, $option );
		}, $typography_options );
	}
}
