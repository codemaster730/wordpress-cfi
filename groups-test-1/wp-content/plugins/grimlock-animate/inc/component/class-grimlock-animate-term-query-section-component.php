...
/**
 * Class Grimlock_Animate_Term_Query_Section_Component
 *
 * @author  themosaurus
 * @since   1.0.0
 * @package grimlock-query/inc/components
 */
class Grimlock_Animate_Term_Query_Section_Component extends Grimlock_Term_Query_Section_Component {
	/**
	 * Setup class.
	 *
	 * @param array $props
	...
	public function __construct( $props = array() ) {
		parent::__construct( wp_parse_args( $props, array(
			'terms_reveal_selector'    => '.section__header, article, .section__footer',
			'terms_reveal'             => 'none',
			'terms_reveal_duration'    => 750,
			'terms_reveal_interval'    => 50,
			'terms_reveal_distance'    => '80px',
			'terms_reveal_delay'       => 500,
			'terms_reveal_rotate_x'    => 0,
			'terms_reveal_rotate_y'    => 0,
			'terms_reveal_rotate_z'    => 0,
			'terms_reveal_opacity'     => 0,
			'terms_reveal_scale'       => 1,
			'terms_reveal_easing'      => 'cubic-bezier(0.6, 0.2, 0.1, 1)',
			'terms_reveal_view_factor' => 0.5,
			'reveal_mobile'            => true,
			'reveal_reset'             => false,
		) ) );
	}
	...
	public function get_data_attributes() {
		...
		return array_merge( $data_attributes, $this->get_element_reveal_data_attributes( 'terms' ) );
	}
	...
	public function get_terms_class( $class = '' ) {
		$classes   = parent::get_terms_class( $class );
		$classes[] = $this->get_element_reveal_class( 'terms' );
