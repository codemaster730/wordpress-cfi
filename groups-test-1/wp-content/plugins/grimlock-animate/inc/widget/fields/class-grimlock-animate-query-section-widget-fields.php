<?php
/**
 * Grimlock_Animate_Query_Section_Widget_Fields Class
 *
 * @author  Themosaurus
 * @since   1.0.0
 * @package  grimlock
 ...
 * @see WP_Widget
 */
class Grimlock_Animate_Query_Section_Widget_Fields extends Grimlock_Base_Widget_Fields {
	/**
	...
	public function __construct( $id_base = 'grimlock_query_section_widget' ) {
		...
		// Animation tab
		add_action( "{$this->id_base}_animation_tab", array( $this, 'add_posts_reveal_field'  ), 100, 2 );
	 ...
	 */
	public function add_posts_reveal_field( $instance, $widget ) {
		$args = array(
			'id'      => $widget->get_field_id( 'posts_reveal' ),
			'name'    => $widget->get_field_name( 'posts_reveal' ),
			'label'   => esc_html__( 'Posts reveal:', 'grimlock-animate' ),
			'value'   => $instance['posts_reveal'],
			'choices' => array(
		...
		$this->select( apply_filters( "{$this->id_base}_posts_reveal_field_args", $args, $instance ) );
	}
	...
	public function change_component_args( $component_args, $instance, $widget_args, $widget_id ) {
		return array_merge( $component_args, array(
			'posts_reveal'  => $instance['posts_reveal'],
	...
	public function sanitize_instance( $new_instance, $old_instance ) {
		...
		$instance['posts_reveal']  = isset( $new_instance['posts_reveal'] ) ? sanitize_text_field( $new_instance['posts_reveal'] ) : '';
...
return new Grimlock_Animate_Query_Section_Widget_Fields();
