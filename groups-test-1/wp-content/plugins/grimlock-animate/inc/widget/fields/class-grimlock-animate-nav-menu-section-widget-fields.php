<?php
/**
 * Grimlock_Animate_Nav_Menu_Section_Widget_Fields Class
 *
 * @author  Themosaurus
 * @since   1.0.0
 * @package  grimlock
 ...
 * @see WP_Widget
 */
class Grimlock_Animate_Nav_Menu_Section_Widget_Fields extends Grimlock_Base_Widget_Fields {
	/**
	...
	public function __construct( $id_base = 'grimlock_nav_menu_section_widget' ) {
		...
		// Animation tab
		add_action( "{$this->id_base}_animation_tab", array( $this, 'add_menu_items_reveal_field'  ), 100, 2 );
	 ...
	 */
	public function add_menu_items_reveal_field( $instance, $widget ) {
		$args = array(
			'id'      => $widget->get_field_id( 'menu_items_reveal' ),
			'name'    => $widget->get_field_name( 'menu_items_reveal' ),
			'label'   => esc_html__( 'Menu Items Reveal:', 'grimlock-animate' ),
			'value'   => $instance['menu_items_reveal'],
			'choices' => array(
		...
		$this->select( apply_filters( "{$this->id_base}_menu_items_reveal_field_args", $args, $instance ) );
	}
	...
	public function change_component_args( $component_args, $instance, $widget_args, $widget_id ) {
		return array_merge( $component_args, array(
			'menu_items_reveal'  => $instance['menu_items_reveal'],
	...
	public function sanitize_instance( $new_instance, $old_instance ) {
		...
		$instance['menu_items_reveal']  = isset( $new_instance['menu_items_reveal'] ) ? sanitize_text_field( $new_instance['menu_items_reveal'] ) : '';
...
return new Grimlock_Animate_Nav_Menu_Section_Widget_Fields();
