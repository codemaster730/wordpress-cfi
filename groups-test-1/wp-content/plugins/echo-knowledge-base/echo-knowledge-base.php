...
final class Echo_Knowledge_Base {
	...
	private function setup_system() {
		...
		// TODO new EPKB_Help_Dialog_View();
	}
	...
	private function handle_ajax_requests( $action ) {
		...
		if ( $action == 'epkb_help_dialog_get_category_list' || $action == 'epkb_help_dialog_article_detail' || $action == 'epkb_help_dialog_search_kb' ) {
			// TODO new EPKB_Help_Dialog_Search();
			return;
		}

		if ( $action == 'epkb_help_dialog_contact' ) {
			// TODO new EPKB_Help_Dialog_Ctrl();
			return;
		}
		...
		if ( in_array($action, ['epkb_enable_help_dialog', 'epkb_create_faq', 'epkb_get_faq', 'epkb_delete_faq', 'epkb_update_faq', 'epkb_update_faq_list']) ) {
			// TODO new EPKB_Help_Dialog_Ctrl_FAQ();
			return;
		}

		if ( in_array($action, ['epkb_create_location', 'epkb_get_location', 'epkb_delete_location', 'epkb_update_location', 'epkb_update_location_list', 'epkb_search_posts']) ) {
			// TODO new EPKB_Help_Dialog_Ctrl_Location();
			return;
		}
	}
	...
	public function setup_backend_classes() {
		...
		/* TODO if ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG && EPKB_KB_Handler::is_kb_request() ) {
