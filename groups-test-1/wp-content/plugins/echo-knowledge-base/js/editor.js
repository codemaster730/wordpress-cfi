...
jQuery(document).ready(function($) {
	...
	window.EPKBEditor = {
		...
		afterLoadIframe: function( event ) {
			...
			app.iframe.find( app.currentSettings[app.preselectZone].classes ).eq(app.preselectEq).trigger('click');
			
			if ( 'help_dialog' == app.preselectZone && app.iframe.find('#eckb-help-dialog').css( 'display' ) == 'none' ) {
				app.iframe.find('.eckb-help-dialog-toggle').trigger('click');
			}
		...
		bindModalEvents: function() {
			...
			// Typography events 
			$(document).on( 'click', '.epkb-editor-settings-typography_button', this.onTypographyEditButtonClick );
		...
		onBackClick: function ( event ) {
			...
			if ( app.activeZone == 'settings_panels' ) {
				...
				app.iframe.find( app.currentSettings[app.lastActiveZone].classes ).trigger('click');
				app.showMenuButtons();
			} else {
				// TODO initial screen 
				app.clearModal();
			...
			if ( $el.closest('#epkb-editor-settings-panel-hidden').length > 0 ) {
				...
				if ( newVal == 'on' ) {
					...
					for ( let optionName in app.currentSettings[name].disabled_settings ) {
						...
						if ( name == 'elay_sidebar_navigation_zone' ) {
							app.updateOption( 'article-left-sidebar-toggle', 'on' );
						}
		...
		onTypographyEditButtonClick: function(e) {
					...
					if ( field.type == 'typography' ) {
						...
						if ( typeof field.value['font-family'] !== 'undefined' ) {
							...
							if ( field.value['font-family'] ) {
								...
								this.iframe.find(field.target_selector).css({'font-family' : ''});
							}
						...
						if ( typeof field.value['font-weight'] !== 'undefined' ) {
							if ( field.value['font-weight'] ) {
								...
								this.iframe.find(field.target_selector).css({'font-weight' : ''});
							}
						...
						if ( typeof field.value['font-size'] !== 'undefined' ) {
							...
							if ( field.value['font-size'] ) {
								...
								this.iframe.find(field.target_selector).css({'font-size' : ''});
							}
						} 
		...
		refreshIframe: function( app ) {
			...
			if ( app.activeZone ) {
				...
				if ( typeof app.currentSettings[app.preselectZone] != 'undefined' ) {
			...
			if ( $('.epkb-editor-settings__panel-navigation__tab--active').data('target') !== 'global' )
		...
		showNavigation: function( disabledTab = false ) {
			...
			for ( let zoneName in app.currentSettings ) {
				...
				if ( zoneName == 'elay_sidebar_navigation_zone' && is_on == true  && app.getOption( 'article-left-sidebar-toggle' ) == 'off' ) {
					is_on = false;
				}
			...
			let html = `
				...
				html += `<div class="epkb-editor-settings-typography_field">
								...
								if ( data.value['font-family'] == '' ) {
									html += `<option value="" selected=selected">${epkb_editor.default}</option>`;
								} else {
									html += `<option value="">${epkb_editor.default}</option>`;
								}
								
								epkb_editor.typography_fonts.forEach( function(fontName) {
									if ( data.value['font-family'] == fontName ) {
										html += `<option value="${fontName}" selected=selected" style="font-family:${fontName};">${fontName}</option>`;
									} else {
										html += `<option value="${fontName}" style="font-family:${fontName};">${fontName}</option>`;
									}
								} );
			html += `</select>
						</div>
						<div class="epkb-editor-settings-typography_field">
							...
							<div class="epkb-editor-settings-control__input">
								<input type="number" name="${data.name}" value="${data.value['font-size']}" min="1" max="100"  data-typography="font-size" placeholder="${epkb_editor.default}">
							</div>
						</div>
						<div class="epkb-editor-settings-typography_field">
							...
							<div class="epkb-editor-settings-control__input">
								<select name="${data.name}" data-typography="font-weight">
									<option value="" ${ ( data.value['font-weight'] == '' ) ? 'selected="selected"' : '' }>${epkb_editor.default}</option>
