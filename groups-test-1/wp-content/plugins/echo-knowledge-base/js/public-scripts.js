...
*/
	...
	let TOC = {
		...
		init: function() {
			...
			$('.eckb-article-toc__level a').on('click', function(){
				...
				$('body, html').animate({ scrollTop: $( '[data-id=' + target + ']').offset().top - that.offset }, 50);
		...
		getToCHTML: function ( headers, titleTag='h4' ) {
			...
			headers.forEach( function( header ) {
				html += `<li class="eckb-article-toc__level eckb-article-toc__level-${header.level}"><a data-target="${header.id}">${header.title}</a></li>`;
			});
	...
	function mobile_TOC() {
		...
		let mobile_breakpoint = typeof articleToc.data('mobile_breakpoint') == "undefined" ? 111 : articleToc.data('mobile_breakpoint');
		...
		if ( window_width <= mobile_breakpoint ) {
			if (!$('#eckb-article-content-header-v2 .eckb-article-toc').length) {
				...
				} else if ($('#eckb-article-right-sidebar .eckb-article-toc').length) {
