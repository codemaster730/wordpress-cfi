...
class EPKB_Settings_DB {
	...
	public function get_settings() {
		...
		if ( ! empty($this->cached_settings) ) {
			$settings = wp_parse_args( $this->cached_settings, EPKB_Settings_Specs::get_default_settings() );
			return $settings;
		}
		...
		$settings = EPKB_Editor_Controller::filter_plugin_settings( $settings );
	...
	private function save_settings( $settings ) {
		...
		if ( $result === false ) {
			EPKB_Logging::add_log( 'Failed to update Plugin settings', 'Last DB ERROR: (' . $wpdb->last_error . ')' );
			return new WP_Error( 'save_kb_config', 'Failed to update Plugin settings. Last DB ERROR: (' . $wpdb->last_error . ')');
		}
