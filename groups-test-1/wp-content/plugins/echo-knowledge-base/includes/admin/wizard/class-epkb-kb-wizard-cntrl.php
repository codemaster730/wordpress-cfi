...
class EPKB_KB_Wizard_Cntrl {
	...
	public function apply_wizard_changes() {
		...
		$new_config_post = EPKB_Utilities::post( 'kb_config', array(), false );
		...
		switch( $wizard_type ) {
			case 'theme':
				$this->apply_theme_wizard_changes( $wizard_kb_id, $orig_config, $new_config );
				break;
			case 'text':
			case 'search':
				$this->apply_non_default_wizard_changes( $orig_config, $new_config );
				break;
			case 'features':
				$this->apply_features_wizard_changes( $orig_config, $new_config );
				break;
			case 'ordering':
	 ...
	 * @param $new_config
	 */
	private function apply_theme_wizard_changes( $wizard_kb_id, $orig_config, $new_config ) {
		...
		$article_sidebar_component_priority = EPKB_Utilities::post('article_sidebar_component_priority', array(), false);
	 ...
	 * @param $new_config
	 */
	private function apply_non_default_wizard_changes( $orig_config, $new_config ) {
	 ...
	 * @param $new_config
	 */
	private function apply_features_wizard_changes( $orig_config, $new_config ) {
	...
	public function update_kb_configuration( $kb_id, $orig_config, $new_config ) {
		...
		// enable left sidebar if needed for specific Layout if user changed to that layout
		if ( $orig_config['kb_main_page_layout'] != $new_config['kb_main_page_layout'] ) {
			if ( in_array($new_config['kb_main_page_layout'], [ EPKB_KB_Config_Layouts::CATEGORIES_LAYOUT, EPKB_KB_Config_Layouts::GRID_LAYOUT, EPKB_KB_Config_Layouts::SIDEBAR_LAYOUT]) ) {
	...
	public function wizard_update_order_view() {
		...
		$sequence_settings = EPKB_Utilities::post( 'sequence_settings', array(), false );
