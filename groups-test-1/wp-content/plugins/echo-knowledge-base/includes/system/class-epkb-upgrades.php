...
class EPKB_Upgrades {
	...
	private static function upgrade_to_v770( &$kb_config ) {
		...
		if ( ! empty($kb_config['categories_box_font_size']) ) {
			$kb_config['categories_box_typography'] = array_merge( EPKB_Editor_Utilities::$typography_defaults, array( 'font-size' => $kb_config['categories_box_font_size'] ) );
		}
		
		if ( ! empty($kb_config['article_search_title_font_size']) ) {
			$kb_config['article_search_title_typography'] = array_merge( EPKB_Editor_Utilities::$typography_defaults, array( 'font-size' => $kb_config['article_search_title_font_size'] ) );
		}
		
		if ( ! empty($kb_config['article_toc_font_size']) ) {
			$kb_config['article_toc_typography'] = array_merge( EPKB_Editor_Utilities::$typography_defaults, array( 'font-size' => $kb_config['article_toc_font_size'] ) );
			$kb_config['article_toc_header_typography'] = array_merge( EPKB_Editor_Utilities::$typography_defaults, array( 'font-size' => $kb_config['article_toc_font_size'] ) );
		}
		
		if ( ! empty($kb_config['back_navigation_font_size']) ) {
			$kb_config['back_navigation_typography'] = array_merge( EPKB_Editor_Utilities::$typography_defaults, array( 'font-size' => $kb_config['back_navigation_font_size'] ) );
		}
		if ( ! empty($kb_config['breadcrumb_font_size']) ) {
			$kb_config['breadcrumb_typography'] = array_merge( EPKB_Editor_Utilities::$typography_defaults, array( 'font-size' => $kb_config['breadcrumb_font_size'] ) );
		}
		...
		if ( ! empty($kb_config['section_font_size']) ) {
			...
			$kb_config['section_typography'] = array_merge( EPKB_Editor_Utilities::$typography_defaults, array( 'font-size' => $section_font_size ) );
			$kb_config['section_head_typography'] = array_merge( EPKB_Editor_Utilities::$typography_defaults, array( 'font-size' => $section_head_typography ) );
			$kb_config['section_head_description_typography'] = array_merge( EPKB_Editor_Utilities::$typography_defaults, array( 'font-size' => $section_head_description_typography ) );
		}
		...
		if ( ! empty($kb_config['tab_font_size']) ) {
			...
			$kb_config['tab_typography'] = array_merge( EPKB_Editor_Utilities::$typography_defaults, array( 'font-size' => $tab_font_size ) );
		}

		if ( ! empty($kb_config['search_title_font_size']) ) {
			$kb_config['search_title_typography'] = array_merge( EPKB_Editor_Utilities::$typography_defaults, array( 'font-size' => $kb_config['search_title_font_size'] ) );
		}
