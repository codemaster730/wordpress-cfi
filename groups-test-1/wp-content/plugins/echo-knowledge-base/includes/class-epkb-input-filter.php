...
class EPKB_Input_Filter {
		...
		foreach ($input as $key => $input_value) {
			...
			switch ( $type ) {
				...
				case self::TYPOGRAPHY:
					$input_value = EPKB_Editor_Utilities::sanitize_typography( $input_value );
					break;
	...
	private function filter_typography( $value ) {
		...
		if ( ! is_array( $value ) ) {
			return new WP_Error( 'filter_not_typograghy', sprintf( __( 'The value "%s" is not text typography', 'echo-knowledge-base' ), $value ));
		}
		
		if ( ! empty($value['font-family']) && ! in_array($value['font-family'], EPKB_Editor_Utilities::$google_fonts) ) {
			$value['font-family'] = EPKB_Editor_Utilities::$typography_defaults['font-family'];
		}

		if ( ! empty($value['font-size']) && ( $value['font-size'] < 4 || $value['font-size'] > 100 ) ) {
			$value['font-size'] = EPKB_Editor_Utilities::$typography_defaults['font-size'];
		}

		if ( ! in_array($value['font-size-units'], ['px', 'em']) ) {
			$value['font-size-units'] = EPKB_Editor_Utilities::$typography_defaults['font-size-units'];
		}

		if ( ! empty($value['font-weight']) && ! in_array($value['font-weight'], [100, 200, 300, 400, 500, 600, 700, 800, 900]) ) {
			$value['font-weight'] = EPKB_Editor_Utilities::$typography_defaults['font-weight'];
		}
	...
	public function retrieve_and_sanitize_form_fields( $submitted_fields, $all_fields_specs, $orig_settings ) {
		...
		foreach ($all_fields_specs as $key => $spec ) {
			...
			if ( $spec['type'] == self::WP_EDITOR ) {
				...
				$name_values += array( $key => EPKB_Editor_Utilities::sanitize_typography( $input_value ) );
