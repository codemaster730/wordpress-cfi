...
if ( ! empty( $group_types ) ) : ?>
	...
	<fieldset class="group-create-types mb-3">
		...
		<?php foreach ( $group_types as $type ) : ?>
			<div class="checkbox">
				<label for="<?php echo esc_attr( sprintf( 'group-type-%s', $type->name ) ); ?>">
					<input type="checkbox" name="group-types[]" id="<?php echo esc_attr( sprintf( 'group-type-%s', $type->name ) ); ?>" value="<?php echo esc_attr( $type->name ); ?>" <?php checked( bp_groups_has_group_type( bp_get_current_group_id(), $type->name ) ); ?>/> <?php echo esc_html( $type->labels['name'] ); ?>
					<?php
