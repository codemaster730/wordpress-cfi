...
class EPKB_Convert {
	...
	public static function get_posts_table( $posts, $taxonomies ) {
		...
		$table_header = [
			...
			esc_html__( 'Status', 'echo-knowledge-base' ),
			esc_html__( 'Post Title', 'echo-knowledge-base' ),
			esc_html__( 'Taxonomies', 'echo-knowledge-base' ),
		];
		...
		foreach ( $posts as $post ) {
			...
			$input_box = "<input type='checkbox' name='row_id' value='" . esc_attr( $post->ID ) . "' checked>";
			...
			foreach ( $taxonomies as $tax_slug => $tax ) {
				...
				if ( is_wp_error( $terms ) ) {
					return '<div class="epkb-no-results">' . esc_html__( 'Error Occurred', 'echo-knowledge-base' ) . $terms->get_error_message() . '</div>';
				}
	...
	public static function display_import_table( $title, $description, $table_header, $table_rows, $type, $taxonomies, $learn_more = '' ) {
		...
		foreach ( $table_header as $table_header_item ) {
			$response_html .= '<span>' . $table_header_item . '</span>';  // sanitized early on
		}
		$response_html .= '</div>';
		...
		foreach ( $table_rows as $rows ) {
			...
			foreach ( $rows as $row_id => $row ) {
				$response_html .= '<span class="' . $row_id . '">' . $row . '</span>';  // sanitized early on
			}
			$response_html .= '</div>';
