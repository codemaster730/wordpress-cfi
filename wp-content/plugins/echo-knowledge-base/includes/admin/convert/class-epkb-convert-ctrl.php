...
class EPKB_Convert_Ctrl {
	...
	public function load_articles_list() {
		...
		wp_die( json_encode( array( 'success' => $output_messages, 'response_html_1' => $response_html_1, 'response_html_2' => $response_html_2 ) ) );
	}
	...
	public function convert_kb_content() {
		...
		if ( empty( $post_type ) || ! post_type_exists( $post_type ) ) {
			EPKB_Utilities::ajax_show_error_die( EPKB_Utilities::report_generic_error( 343, __( 'Refresh your page', 'echo-knowledge-base' ) ), '', 343 );
		}

		$step = (int)EPKB_Utilities::post( 'epkb_convert_step' );
		if ( $step != 4 ) {
			EPKB_Utilities::ajax_show_error_die( EPKB_Utilities::report_generic_error( 344, __( 'Refresh your page', 'echo-knowledge-base' ) ), '', 344 );
		}
		...
		if ( empty( $selected_rows ) || ! is_array( $selected_rows ) ) {
			EPKB_Utilities::ajax_show_error_die( EPKB_Utilities::report_generic_error( 344, __( 'Get Selected Rows', 'echo-knowledge-base' ), __( 'No articles were selected for import.', 'echo-knowledge-base' ) ), '', 344 );
		}
		...
		if ( ! in_array( $convert_terms_mode, [ 'copy_terms', 'remove_terms' ] ) ) {
			EPKB_Utilities::ajax_show_error_die( EPKB_Utilities::report_generic_error( 345, __( 'Refresh your page', 'echo-knowledge-base' ) ), '', 345 );
		}
					...
					if ( empty( $new_term_id ) ) {
						...
						if ( empty( $new_term ) || is_wp_error( $new_term ) ) {
							...
							if ( $convert_terms_mode == 'remove_terms' ) {
								EPKB_Utilities::ajax_show_error_die( EPKB_Utilities::report_generic_error( 355, __( 'Can not create term', 'echo-knowledge-base' ) . ': ' . $old_term->name ), '', 355 );
							}
		...
		wp_die( json_encode( array( 'success' => $output_message, 'inserted' => $processed_articles_count, 'process_errors' => $errors_text ) ) );
	}
}