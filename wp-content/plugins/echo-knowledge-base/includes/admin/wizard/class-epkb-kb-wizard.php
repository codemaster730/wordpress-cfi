...
class EPKB_KB_Wizard {
	...
	public function display_kb_wizard( $kb_config ) {
		...
		$this->templates = EPKB_KB_Wizard_Themes::get_all_presets( $kb_config );
				...
				<div class="epkb-wizard-header">
					<div class="epkb-wizard-header__info">
						<h1 class="epkb-wizard-header__info__title">
							<?php _e( 'Color and Style Settings', 'echo-knowledge-base' ); ?>
						</h1>
		...
		<div id="epkb-wsb-step-2-panel" class="epkb-wc-step-panel eckb-wizard-step-2">
			<div class="epkb-wizard-theme-preview">
					...
					<ul>						<?php
						...
						<li id="epkb-wt-theme-<?php echo $first_theme_id; ?>" class="epkb-wt-tab epkb-wt--active epkb-wt--current epkb-wt-theme-first" data-template_id="<?php echo $first_theme_id; ?>" >
							...
							<div class="epkb-wt-theme-first__name">
								...
								echo  __('Saved Configuration', 'echo-knowledge-base'); ?></span>
								<div class="epkb-wt-theme-first__desc">								<?php
									echo __('Your last saved KB configuration as seen on the front end.', 'echo-knowledge-base'); ?>
								</div>
							</div>
				...
				<div class="epkb-wizard-theme-panel-container">					<?php
					...
					$first_theme_id = 'current';
					$first_theme_config = $this->kb_config; ?>
	...
	private function wizard_step_main_page_colors() {
		...
		$is_left_sidebar_on = EPKB_Articles_Setup::is_left_sidebar_on( $this->kb_config );
		$is_right_sidebar_on = EPKB_Articles_Setup::is_right_sidebar_on( $this->kb_config  );
	...
	private function wizard_buttons() {
		// TODO remove
		if (  self::is_wizard_disabled() ) {
			return;
		}   ?>
    ...
    private static function is_wizard_disabled() {

		if ( defined( 'WIZARD_DEBUG' ) && WIZARD_DEBUG ) return false;

		if ( class_exists( 'Echo_Elegant_Layouts') && version_compare(Echo_Elegant_Layouts::$version, '2.2.0', '<' ) ) {
			return true;
		}

	    if ( class_exists( 'Echo_Advanced_Search' ) && version_compare(Echo_Advanced_Search::$version, '2.7.0', '<' ) ) {
		    return true;
	    }

	    if ( class_exists( 'Echo_Article_Rating_And_Feedback' ) && version_compare(Echo_Article_Rating_And_Feedback::$version, '1.1.0', '<' ) ) {
		    return true;
	    }
	 ...
	 * @param $args
	 */
	private function wizard_section( $hook, $args ) {
		do_action( $hook, $args );
	}
	...
	private function get_divided_templates() {
		...
		if ( is_array( $this->templates) ) {
			foreach ( $this->templates as $template_id => $template ) {
				if ( isset( $template['kb_main_page_layout'] ) ) {
					$divided_templates[$template['kb_main_page_layout']][$template_id] = $template;
				} else {
	...
	private static function show_demo_articles_categories_alert() {
