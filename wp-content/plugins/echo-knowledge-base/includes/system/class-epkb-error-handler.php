...
class EPKB_Error_Handler {
	...
	public static function js_not_loaded() {
		...
		if ( ! EPKB_Core_Utilities::is_kb_flag( 'editor_backend_mode' ) ) { ?>
			...
			<a class="epkb-editor-safe-mode-link" href="<?php echo admin_url( '/edit.php?post_type=' . EPKB_KB_Handler::get_post_type( EPKB_KB_Handler::get_current_kb_id() ) . '&page=epkb-kb-configuration&action=enable_editor_backend_mode&_wpnonce_manage_kbs=' .
                                                                  wp_create_nonce( '_wpnonce_manage_kbs' ) . '#kb-design' ); ?>"><?php esc_html_e( 'Click here to go to the Configuration Page and turn on Editor back-end mode.', 'echo-knowledge-base' ); ?></a>
			<p><?php
