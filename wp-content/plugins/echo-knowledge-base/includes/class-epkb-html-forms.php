...
class EPKB_HTML_Forms {
			...
			if ( ! empty( $box_options['title'] ) ){    ?>
				...
				?>">
					...
					if ( ! empty( $box_options['tooltip_title'] ) && ! empty( $box_options['tooltip_desc'] ) ) {
						...
						EPKB_HTML_Admin::display_tooltip( $box_options['tooltip_title'], $box_options['tooltip_desc'], $box_options['tooltip_args'] );
					} ?>
                </h4>   <?php
			}
	...
	public static function get_feature_box_html( $feature ) {
		...
		<div class="epkb-kbnh__feature-container__col epkb-kbnh__feature__content-col">
			...
			<div class="epkb-kbnh__feature-links">  <?php
				...
				if ( ! empty( $feature['configure_link'] ) ) { ?>
					<a class="epkb-kbnh__feature-link" href="<?php echo $feature['configure_link']; ?>" target="_blank"><span><?php esc_html_e( 'Configure', 'echo-knowledge-base' ); ?></span></a>    <?php
				}

				if ( ! empty( $feature['docs'] ) ) { ?>
					<a class="epkb-kbnh__feature-link" href="<?php echo $feature['docs']; ?>"
					   target="_blank"><span><?php esc_html_e( 'Docs', 'echo-knowledge-base' ); ?></span></a>    <?php
				}

				// Link to Video Tutorial
				if ( ! empty( $feature['video'] ) ) { ?>
					<a class="epkb-kbnh__feature-link" href="<?php echo $feature['video']; ?>"
					   target="_blank"><span><?php esc_html_e( 'Video Tutorial', 'echo-knowledge-base' ); ?></span></a>    <?php
				}

				// Learn More
				if ( ! empty( $feature['learn_more'] ) ) { ?>
					<a class="epkb-kbnh__feature-link" href="<?php echo $feature['learn_more']; ?>"
		...
		<div class="epkb-kbnh__feature-container__col epkb-kbnh__feature__status-col">    <?php
			...
			if ( ! empty( $feature['active_status'] ) ) {
				echo '<span class="epkb-kbnh__feature-status epkb-kbnh__feature--installed">' . esc_html__( 'Active', 'echo-knowledge-base' ) . '</span>';
			// Plugin is not enabled
				...
				echo '<a class="epkb-kbnh__feature-status epkb-kbnh__feature--disabled" href="' . EPKB_Core_Utilities::get_plugin_sales_page( $feature['plugin'] ) . '" target="_blank"><span>' . esc_html__( 'Install', 'echo-knowledge-base' ) . '</span></a>';
			} ?>
	...
	private static function display_item_column_fields( $item, $item_column_fields, $item_primary_key ) {   ?>
		...
		<tr class="epkb-admin__items-list__column-fields">     <?php
			...
			foreach ( $item_column_fields as $field_key => $field_title ) {   ?>
				<td class="epkb-admin__items-list__column-field"><?php echo esc_html( $item->$field_key ); ?></td>  <?php
			}   ?>
