...
<?php do_action( 'tribe_events_bar_before_template' ); ?>
	<div id="tribe-events-bar">
		...
		<form id="tribe-bar-form" class="tribe-clearfix" name="tribe-bar-form" method="post" action="<?php echo esc_url( $current_url ); ?>">
			...
			<div id="tribe-bar-collapse-toggle" <?php echo count( $views ) === 1 ? 'class="tribe-bar-collapse-toggle-full-width"' : ''; ?>>
				<?php
				/* translators: %s: Events plural label */
				printf( esc_html__( 'Find %s', 'the-events-calendar' ), tribe_get_event_label_plural() ); ?><span class="tribe-bar-toggle-arrow"></span>
			</div>
					...
					<div class="tribe-bar-filters-inner tribe-clearfix">
						<?php foreach ( $filters as $filter ) : ?>
							<div class="<?php echo esc_attr( $filter['name'] ); ?>-filter">
								...
								<?php
								echo $filter['html']; // We don't escape this one to avoid breaking the plugin filters (we left it as it is in the original plugin template)
								?>
							</div>
						<?php endforeach; ?>
						<div class="tribe-bar-submit">
							...
							/* translators: %s: Events plural label */
							printf( esc_attr__( 'Find %s', 'the-events-calendar' ), tribe_get_event_label_plural() ); ?>" />
