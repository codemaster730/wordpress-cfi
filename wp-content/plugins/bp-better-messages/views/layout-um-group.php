...
$is_in_groups_now = ! is_page() && ! wp_doing_ajax();
...
$members_count    = um_groups_get_member_count( $group_id, true );

$participants = BP_Better_Messages()->functions->get_participants( $thread_id );

if( (int) $members_count !== (int) $participants['count'] ){
    Better_Messages_Ultimate_Member_Groups::instance()->sync_thread_members($thread_id);
    $participants = BP_Better_Messages()->functions->get_participants( $thread_id );
}

if( ! $group ){
    BP_Better_Messages()->functions->delete_thread_meta( $thread_id, 'um_group_id' );
    BP_Better_Messages()->functions->delete_thread_meta( $thread_id, 'um_group_thread' );
}

$has_access  = Better_Messages_Ultimate_Member_Groups::instance()->user_has_access( $group_id, $user_id );

$link        = get_permalink( $group->ID );
$subject     = BP_Better_Messages()->functions->get_thread_subject($thread_id);


if( trim( esc_html( $group->post_title ) ) !== trim($subject) ){
    $subject = BP_Better_Messages()->functions->change_thread_subject( $thread_id, trim( esc_html( $group->post_title )) );
...
?><div class="bp-messages-wrap <?php if( $is_in_groups_now ) { echo 'bp-messages-group-thread'; } else { echo 'bp-messages-wrap-main'; }; ?> <?php BP_Better_Messages()->functions->messages_classes($thread_id, 'group'); ?>" data-thread-id="<?php esc_attr_e($thread_id); ?>">
    ...
    <div class="bpbm-chat-content">
        ...
        <?php if( apply_filters('bp_better_messages_can_send_message', $has_access, get_current_user_id(), $thread_id ) ) { ?>
        <div class="reply">
