...
if( (int) $group->members_count !== (int) $participants['count'] ){
    BP_Better_Messages_Peepso_Groups::instance()->sync_thread_members($thread_id);
    $participants = BP_Better_Messages()->functions->get_participants( $thread_id );
}

if( $group->id === NULL ){
    BP_Better_Messages()->functions->delete_thread_meta( $thread_id, 'peepso_group_id' );
    BP_Better_Messages()->functions->delete_thread_meta( $thread_id, 'peepso_group_thread' );
}

$has_access  = BP_Better_Messages_Peepso_Groups::instance()->user_has_access( $group->id, get_current_user_id() );
