/*! bmselectise.js - v0.13.4 | https://github.com/bmselectise/bmselectise.js | Apache License (v2) */
!function (root, factory) {
    ...
    "function" == typeof define && define.amd ? define("bmselectise", ["jquery", "sifter", "microplugin"], factory) : "object" == typeof module && "object" == typeof module.exports ? module.exports = factory(require("jquery"), require("sifter"), require("microplugin")) : root.Bmselectise = factory(root.jQuery, root.Sifter, root.MicroPlugin)
    ...
    function autoGrow($input) {
        function update(e, options) {
            ...
            options = options || {}, (e = e || window.event || {}).metaKey || e.altKey || !options.force && !1 === $input.data("grow") || (options = $input.val(), e.type && "keydown" === e.type.toLowerCase() && (printable = 48 <= (keyCode = e.keyCode) && keyCode <= 57 || 65 <= keyCode && keyCode <= 90 || 96 <= keyCode && keyCode <= 111 || 186 <= keyCode && keyCode <= 222 || 32 === keyCode, 46 === keyCode || 8 === keyCode ? (selection = getSelection($input[0])).length ? options = options.substring(0, selection.start) + options.substring(selection.start + selection.length) : 8 === keyCode && selection.start ? options = options.substring(0, selection.start - 1) + options.substring(selection.start + 1) : 46 === keyCode && void 0 !== selection.start && (options = options.substring(0, selection.start) + options.substring(selection.start + 1)) : printable && (keyCode = e.shiftKey, selection = String.fromCharCode(e.keyCode), options += selection = keyCode ? selection.toUpperCase() : selection.toLowerCase())), printable = $input.attr("placeholder"), e = $input, (selection = ((keyCode = options = !options && printable ? printable : options) ? (Bmselectise.$testInput || (Bmselectise.$testInput = $("<span />").css({
                ...
                height: 0,
                overflow: "hidden"
            }).append(Bmselectise.$testInput).appendTo("body")), Bmselectise.$testInput.text(keyCode), transferStyles(e, Bmselectise.$testInput, ["letterSpacing", "fontSize", "fontFamily", "fontWeight", "textTransform"]), Bmselectise.$testInput.width()) : 0) + 4) !== currentWidth && (currentWidth = selection, $input.width(selection), $input.triggerHandler("resize")))
        }
    ...
    var options, IS_MAC = /Mac/.test(navigator.userAgent), KEY_CMD = IS_MAC ? 91 : 17, KEY_CTRL = IS_MAC ? 18 : 17,
        ...
        hook = {
            ...
            $to.css(styles)
        }, Bmselectise = function ($input, settings) {
            var i, n, fn, delay, timeout, input = $input[0],
                computedStyle = (input.bmselectise = this, window.getComputedStyle && window.getComputedStyle(input, null));
            if (computedStyle = (computedStyle ? computedStyle.getPropertyValue("direction") : input.currentStyle && input.currentStyle.direction) || $input.parents("[dir]:first").attr("dir") || "", $.extend(this, {
                ...
                eventNS: ".bmselectise" + ++Bmselectise.count,
    ...
    return MicroEvent.mixin(Bmselectise), void 0 !== MicroPlugin ? MicroPlugin.mixin(Bmselectise) : (MicroPlugin = "Dependency MicroPlugin is missing", options = (options = {explanation: 'Make sure you either: (1) are using the "standalone" version of Bmselectise, or (2) require MicroPlugin before you load Bmselectise.'}) || {}, console.error("Bmselectise: " + MicroPlugin), options.explanation && (console.group && console.group(), console.error(options.explanation), console.group && console.groupEnd())), $.extend(Bmselectise.prototype, {
        setup: function () {
            var $parent, inputId, fn, self = this, settings = self.settings, eventNS = self.eventNS,
                $window = $(window), $document = $(document), $input = self.$input, inputMode = self.settings.mode,
            ...
            }), self.updateOriginalInput(), self.refreshItems(), self.refreshState(), self.updatePlaceholder(), self.isSetup = !0, $input.is(":disabled") && self.disable(), self.on("change", this.onChange), $input.data("bmselectise", self), $input.addClass("bmselectised"), self.trigger("initialize"), !0 === settings.preload && self.onSearchChange("")
            ...
            this.settings.render = $.extend({}, {
                optgroup: function (data) {
                    ...
                    return '<div class="option ' + ("" === data[field_value] ? "bmselectise-dropdown-emptyoptionlabel" : "") + '">' + escape(data[field_label]) + "</div>"
            ...
            if (settings.score && "function" != typeof (calculateScore = this.settings.score.apply(this, [query]))) throw new Error('Bmselectise "score" setting must be a function that returns a function');
            ...
            var i, j, k, groups, groups_order, option, option_html, optgroup, optgroups, html, html_children,
                has_create_option, $active, $create, d, tmp, self = this, query = self.$control_input.val().trim(),
                results = self.search(query), $dropdown_content = self.$dropdown_content,
            ...
            var fn, called,
                setup = "function" == typeof self.settings.create ? this.settings.create : function (input) {
                    var data = {}, input = data[self.settings.labelField] = input;
                    if (self.settings.formatValueToKey && "function" == typeof self.settings.formatValueToKey && (null == (input = self.settings.formatValueToKey.apply(this, [input])) || "object" == typeof input || "function" == typeof input)) throw new Error('Bmselectise "formatValueToKey" setting must be a function that returns a value other than object or function.');
            ...
            var caret = Math.min(this.caretPos, this.items.length), $el = $el[0],
            ...
            this.trigger("destroy"), this.off(), this.$wrapper.remove(), this.$dropdown.remove(), this.$input.html("").append(revertSettings.$children).removeAttr("tabindex").removeClass("bmselectised").attr({tabindex: revertSettings.tabindex}).show(), this.$control_input.removeData("grow"), this.$input.removeData("bmselectise"), 0 == --Bmselectise.count && Bmselectise.$testInput && (Bmselectise.$testInput.remove(), Bmselectise.$testInput = void 0), $(window).off(eventNS), $(document).off(eventNS), $(document.body).off(eventNS), delete this.$input[0].bmselectise
    ...
    }), Bmselectise.count = 0, Bmselectise.defaults = {
        options: [],
        optgroups: [],
        plugins: [],
        delimiter: ",",
        ...
        mode: null,
        wrapperClass: "bmselectise-control",
        inputClass: "bmselectise-input",
        dropdownClass: "bmselectise-dropdown",
        dropdownContentClass: "bmselectise-dropdown-content",
        dropdownParent: null,
        copyClassesToDropdown: !0,
        render: {}
    }, $.fn.bmselectise = function (settings_user) {
        ...
        var defaults = $.fn.bmselectise.defaults, settings = $.extend({}, defaults, settings_user),
        ...
        return this.each(function () {
            ...
            this.bmselectise || ($input = $(this), tag_name = this.tagName.toLowerCase(), (placeholder = $input.attr("placeholder") || $input.attr("data-placeholder")) || settings.allowEmptyOption || (placeholder = $input.children('option[value=""]').text()), settings.allowEmptyOption && settings.showEmptyOptionInDropdown && !$input.children('option[value=""]').length && (input_html = $input.html(), $input.html('<option value="">' + settings.emptyOptionLabel + "</option>" + input_html)), ("select" === tag_name ? init_select : function ($input, settings_element) {
                ...
                optgroups: [],
                items: []
            }), new Bmselectise($input, $.extend(!0, {}, defaults, input_html, settings_user)))
        })
    }, $.fn.bmselectise.defaults = Bmselectise.defaults, $.fn.bmselectise.support = {validity: SUPPORTS_VALIDITY_API}, Bmselectise.define("auto_position", function () {
        ...
        this.positionDropdown = function () {
            ...
            dropdownHeight === POSITION.top ? (Object.assign(styles, {
                ...
                top: "unset",
                margin: "0 0 5px 0"
            }), this.$dropdown.addClass("bmselectise-position-top")) : (Object.assign(styles, {
                top: offset.top,
                bottom: "unset",
                margin: "5px 0 0 0"
            }), this.$dropdown.removeClass("bmselectise-position-top")), this.$dropdown.css(styles)
        }
    }), Bmselectise.define("auto_select_on_type", function (options) {
    ...
    }), Bmselectise.define("autofill_disable", function (options) {
    ...
    }), Bmselectise.define("drag_drop", function (options) {
    ...
    }), Bmselectise.define("dropdown_header", function (options) {
        var original, self = this;
        options = $.extend({
            title: "Untitled",
            headerClass: "bmselectise-dropdown-header",
            titleRowClass: "bmselectise-dropdown-header-title",
            labelClass: "bmselectise-dropdown-header-label",
            closeClass: "bmselectise-dropdown-header-close",
            html: function (data) {
    ...
    }), Bmselectise.define("optgroup_columns", function (options) {
        function equalizeSizes() {
            ...
            if (n && self.$dropdown_content.width()) {
                ...
                options.equalizeWidth && (width_parent = self.$dropdown_content.innerWidth() - getScrollbarWidth(), width = Math.round(width_parent / n), $optgroups.css({width: width}), 1 < n && (width_parent = width_parent - width * (n - 1), $optgroups.eq(n - 1).css({width: width_parent})))
            }
        }
    ...
    }), Bmselectise.define("remove_button", function (options) {
        options = $.extend({
            label: "&times;",
            title: "Remove",
            className: "remove",
    ...
    }), Bmselectise.define("restore_on_backspace", function (options) {
        var original, self = this;
    ...
    }), Bmselectise.define("select_on_focus", function (options) {
    ...
    }), Bmselectise.define("tag_limit", function (options) {
        const self = this;
        options.tagLimit = options.tagLimit, this.onBlur = function () {
            const original = self.onBlur;
            return function (e) {
                if (original.apply(this, e), e) {
                    const $control = this.$control, $items = $control.find(".item"), limit = options.tagLimit;
            ...
            return function (e) {
                if (original.apply(this, e), e) {
                    const $control = this.$control, $items = $control.find(".item");
