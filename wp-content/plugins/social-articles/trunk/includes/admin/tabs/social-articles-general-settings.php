...
<?php
    ...
    $status = "";
    $msg = "";
    if(!function_exists("friends_get_friend_user_ids") || !function_exists('bp_notifications_add_notification')){
        $status="disabled";
        $msg=__("To use this feature, you need to activate BP Friend Connections module and Notifications module.", "social-articles");
    }
    ...
    $rows[] = array(
        'id'      => 'show_to_logged_out_users',
        'label'   => __('Show articles in logged out users profile','social-articles'),
        'content' => $socialArticles->select( 'show_to_logged_out_users', array(
            'false' => __('False', 'social-articles'),
            'true'  => __('True', 'social-articles'),
        ), false
        ),
