...
class HssoIDPMetadataReader{
	...
	public function __construct(DOMNode $xml = NULL){
        ...
        $entitiesDescriptor = HssoUtilities::xpQuery($xml, './saml_metadata:EntitiesDescriptor');

        if(!empty($entitiesDescriptor))
            $entityDescriptors = HssoUtilities::xpQuery($entitiesDescriptor[0], './saml_metadata:EntityDescriptor');
        else
            $entityDescriptors = HssoUtilities::xpQuery($xml, './saml_metadata:EntityDescriptor');

        foreach ($entityDescriptors as $entityDescriptor) {
            $idpSSODescriptor = HssoUtilities::xpQuery($entityDescriptor, './saml_metadata:IDPSSODescriptor');

            if(isset($idpSSODescriptor) && !empty($idpSSODescriptor)){
                array_push($this->identityProviders,new HssoIdentityProviders($entityDescriptor));
            }
...
class HssoIdentityProviders{
	...
	public function __construct(DOMElement $xml = NULL){
        ...
        $idpSSODescriptor = HssoUtilities::xpQuery($xml, './saml_metadata:IDPSSODescriptor');
        ...
        $info = HssoUtilities::xpQuery($xml, './saml_metadata:Extensions');
	...
	private function parseInfo($xml){
		$displayNames = HssoUtilities::xpQuery($xml, './mdui:UIInfo/mdui:DisplayName');
	...
	private function parseSSOService($xml){
		$ssoServices = HssoUtilities::xpQuery($xml, './saml_metadata:SingleSignOnService');
	...
	private function parseSLOService($xml){
		$sloServices = HssoUtilities::xpQuery($xml, './saml_metadata:SingleLogoutService');
	...
	private function parsex509Certificate($xml){
		foreach ( HssoUtilities::xpQuery($xml, './saml_metadata:KeyDescriptor') as $KeyDescriptorNode ) {
	...
	private function parseSigningCertificate($xml){
		$certNode = HssoUtilities::xpQuery($xml, './ds:KeyInfo/ds:X509Data/ds:X509Certificate');
		$certData = trim($certNode[0]->textContent);
		$certData = str_replace(array ( "\r", "\n", "\t", ' '), '', $certData);
		if(!empty($certNode))
			array_push($this->signingCertificate, HssoUtilities::sanitize_certificate( $certData ));
	}
