...
class HssoCustomer {
	...
	function create_customer() {
		$url = mo_hsso_options_plugin_constants::HOSTNAME . '/moas/rest/customer/add';
		...
		$fields = array (
				'areaOfInterest' => 'Headless Single Sign On Plugin',
				'email' => $this->email,
				'password' => $password
		);
		...
		$response = HssoUtilities::mo_hsso_wp_remote_post($url, $args);
		return $response;

	}

	function get_customer_key() {
		$url = mo_hsso_options_plugin_constants::HOSTNAME . "/moas/rest/customer/key";
	...
	function check_customer() {
		$url = mo_hsso_options_plugin_constants::HOSTNAME . "/moas/rest/customer/check-if-exists";
    ...
    function submit_contact_us($email, $phone, $query, $call_setup) {
        $url = mo_hsso_options_plugin_constants::HOSTNAME. '/moas/rest/customer/contact-us';
        $current_user = wp_get_current_user();

        if($call_setup)
            $query = '[Call Request - Headless SSO] ' . $query ;
        else
            $query = '[Headless SSO Plugin] ' . $query;
	...
	function send_email_alert($email,$phone,$message, $demo_request=false){
		...
		$url = mo_hsso_options_plugin_constants::HOSTNAME . '/moas/api/notify/send';
		...
		$fromEmail			= 'samlsupport@xecurify.com';
		$subject            = "Feedback: Headless Single Sign On Plugin";
		if($demo_request)
			$subject = "DEMO REQUEST: Headless Single Sign On";
		$site_url=site_url();
		...
		$query        = '[Headless SSO Plugin] : ' . $message;
	...
	function mo_hsso_forgot_password($email) {
		$url = mo_hsso_options_plugin_constants::HOSTNAME . '/moas/rest/customer/password-reset';
	...
	function get_timestamp() {
		$url = mo_hsso_options_plugin_constants::HOSTNAME . '/moas/rest/mobile/get-timestamp';
		$response = HssoUtilities::mo_hsso_wp_remote_post($url);
		return $response;
