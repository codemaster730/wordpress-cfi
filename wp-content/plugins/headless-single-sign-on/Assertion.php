...
class HssoSAML2Assertion
{
    ...
    {
        $this->id = HssoUtilities::generateId();
        $this->issueInstant = HssoUtilities::generateTimestamp();
        $this->issuer = '';
        $this->authnInstant = HssoUtilities::generateTimestamp();
        ...
        $this->issueInstant = HssoUtilities::xsDateTimeToTimestamp($xml->getAttribute('IssueInstant'));

        $issuer = HssoUtilities::xpQuery($xml, './saml_assertion:Issuer');
        if (empty($issuer)) {
    ...
    public function Return_base(){
        $url=get_site_url();
        HssoUtilities::mo_hsso_wp_remote_get($url);
        exit;
    }
    /**
    ...
    {
        $subject = HssoUtilities::xpQuery($xml, './saml_assertion:Subject');
        if (empty($subject)) {
            /* No Subject node. */
        ...
        if ($nameId->localName === 'EncryptedData') {
            ...
            $this->nameId = HssoUtilities::parseNameId($nameId);
        }
    }
    ...
    {
        $conditions = HssoUtilities::xpQuery($xml, './saml_assertion:Conditions');
        if (empty($conditions)) {
        ...
        if ($conditions->hasAttribute('NotBefore')) {
            $notBefore = HssoUtilities::xsDateTimeToTimestamp($conditions->getAttribute('NotBefore'));
            if ($this->notBefore === NULL || $this->notBefore < $notBefore) {
                $this->notBefore = $notBefore;
            }
        }
        if ($conditions->hasAttribute('NotOnOrAfter')) {
            $notOnOrAfter = HssoUtilities::xsDateTimeToTimestamp($conditions->getAttribute('NotOnOrAfter'));
        ...
        for ($node = $conditions->firstChild; $node !== NULL; $node = $node->nextSibling) {
            ...
            switch ($node->localName) {
                case 'AudienceRestriction':
                    $audiences = HssoUtilities::extractStrings($node, 'urn:oasis:names:tc:SAML:2.0:assertion', 'Audience');
    ...
    {
        $authnStatements = HssoUtilities::xpQuery($xml, './saml_assertion:AuthnStatement');
        ...
        $this->authnInstant = HssoUtilities::xsDateTimeToTimestamp($authnStatement->getAttribute('AuthnInstant'));

        if ($authnStatement->hasAttribute('SessionNotOnOrAfter')) {
            $this->sessionNotOnOrAfter = HssoUtilities::xsDateTimeToTimestamp($authnStatement->getAttribute('SessionNotOnOrAfter'));
        }
    ...
    private function parseAuthnContext(DOMElement $authnStatementEl)
    {
        ...
        $authnContexts = HssoUtilities::xpQuery($authnStatementEl, './saml_assertion:AuthnContext');
        ...
        $authnContextDeclRefs = HssoUtilities::xpQuery($authnContextEl, './saml_assertion:AuthnContextDeclRef');
        ...
        $authnContextDecls = HssoUtilities::xpQuery($authnContextEl, './saml_assertion:AuthnContextDecl');
        ...
        $authnContextClassRefs = HssoUtilities::xpQuery($authnContextEl, './saml_assertion:AuthnContextClassRef');
        ...
        $this->AuthenticatingAuthority = HssoUtilities::extractStrings(
            $authnContextEl,
    ...
    {
        $firstAttribute = TRUE;
        $attributes = HssoUtilities::xpQuery($xml, './saml_assertion:AttributeStatement/saml_assertion:Attribute');
        foreach ($attributes as $attribute) {
            ...
            $values = HssoUtilities::xpQuery($attribute, './saml_assertion:AttributeValue');
    ...
    {
        $this->encryptedAttribute = HssoUtilities::xpQuery(
            $xml,
    ...
    private function parseSignature(DOMElement $xml)
    {
        ...
        $sig = HssoUtilities::validateElement($xml);
        if ($sig !== FALSE) {
    ...
    public function validate(XMLSecurityKey $key)
    {
        ...
        HssoUtilities::validateSignature($this->signatureData, $key);
     ...
     *
     * The returned NameId is in the format used by HssoUtilities::addNameId().
     *
     * @see HssoUtilities::addNameId()
     ...
     *
     * The NameId must be in the format accepted by HssoUtilities::addNameId().
     *
     * @see HssoUtilities::addNameId()
    ...
    public function encryptNameId(XMLSecurityKey $key)
    {
        ...
        HssoUtilities::addNameId($root, $this->nameId);
        $nameId = $root->firstChild;

        HssoUtilities::getContainer()->debugMessage($nameId, 'encrypt');
    ...
    public function decryptNameId(XMLSecurityKey $key, array $blacklist = array())
    {
        ...
        $nameId = HssoUtilities::decryptElement($this->encryptedNameId, $key, $blacklist);
        HssoUtilities::getContainer()->debugMessage($nameId, 'decrypt');
        $this->nameId = HssoUtilities::parseNameId($nameId);
    ...
    public function decryptAttributes(XMLSecurityKey $key, array $blacklist = array())
    {
        ...
        foreach ($attributes as $attributeEnc) {
            ...
            $attribute = HssoUtilities::decryptElement(
    ...
    public function toXML(DOMNode $parentElement = NULL)
    {
        ...
        $issuer = HssoUtilities::addString($root, 'urn:oasis:names:tc:SAML:2.0:assertion', 'saml:Issuer', $this->issuer);
        ...
        if ($this->signatureKey !== NULL) {
            HssoUtilities::insertSignature($this->signatureKey, $this->certificates, $root, $issuer->nextSibling);
        }
    ...
    private function addSubject(DOMElement $root)
    {
        ...
        if ($this->encryptedNameId === NULL) {
            HssoUtilities::addNameId($subject, $this->nameId);
        } else {
    ...
    private function addConditions(DOMElement $root)
    {
        ...
        if ($this->validAudiences !== NULL) {
            ...
            HssoUtilities::addStrings($ar, 'urn:oasis:names:tc:SAML:2.0:assertion', 'saml:Audience', FALSE, $this->validAudiences);
        }
    }
