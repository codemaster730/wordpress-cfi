<?php
/**
 * Grimlock_Animate_Term_Query_Section_Block Class
 *
 * @author  Themosaurus
...
/**
 * The class that extends the Term Query Section bloc to add animation options
 */
class Grimlock_Animate_Term_Query_Section_Block {
	/**
	...
	/**
	 * Grimlock_Animate_Term_Query_Section_Block constructor.
	 *
	 * @param string $id_base ID of the extended block
	 */
	public function __construct( $id_base = 'grimlock_term_query_section_block' ) {
		$this->id_base = $id_base;

		add_filter( "{$this->id_base}_panels", array( $this, 'change_panels' ), 10, 1 );

		// Animation tab
		add_filter( "{$this->id_base}_animation_panel_fields", array( $this, 'add_terms_reveal_field'  ), 100 );
	...
	/**
	 * Add a select to set the type of reveal used on the terms of the section
	 *
	 * @param array $fields Array of block fields
	 *
	 * @return array Modified array of block fields
	 */
	public function add_terms_reveal_field( $fields ) {
		$fields[] = Grimlock_Base_Block::select_field( apply_filters( "{$this->id_base}_terms_reveal_field_args", array(
			'name'    => 'terms_reveal' ,
			'label'   => esc_html__( 'Terms reveal', 'grimlock-animate' ),
			'choices' => array(
	...
	public function change_component_args( $component_args, $attributes ) {
		...
		return array_merge( $component_args, array(
			'terms_reveal'  => $attributes['terms_reveal'],
	...
	public function sanitize_attributes( $new_attributes ) {
		...
		$attributes['terms_reveal']  = isset( $new_attributes['terms_reveal'] ) ? sanitize_text_field( $new_attributes['terms_reveal'] ) : '';
...
return new Grimlock_Animate_Term_Query_Section_Block();
