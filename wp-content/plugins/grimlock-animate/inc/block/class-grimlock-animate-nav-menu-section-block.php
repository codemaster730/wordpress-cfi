<?php
/**
 * Grimlock_Animate_Nav_Menu_Section_Block Class
 *
 * @author  Themosaurus
...
/**
 * The class that extends the Nav Menu Section bloc to add animation options
 */
class Grimlock_Animate_Nav_Menu_Section_Block {
	/**
	...
	/**
	 * Grimlock_Animate_Nav_Menu_Section_Block constructor.
	 *
	 * @param string $id_base ID of the extended block
	 */
	public function __construct( $id_base = 'grimlock_nav_menu_section_block' ) {
		$this->id_base = $id_base;

		add_filter( "{$this->id_base}_panels", array( $this, 'change_panels' ), 10, 1 );

		// Animation tab
		add_filter( "{$this->id_base}_animation_panel_fields", array( $this, 'add_menu_items_reveal_field'  ), 100 );
	...
	/**
	 * Add a select to set the type of reveal used on the menu items of the section
	 *
	 * @param array $fields Array of block fields
	 *
	 * @return array Modified array of block fields
	 */
	public function add_menu_items_reveal_field( $fields ) {
		$fields[] = Grimlock_Base_Block::select_field( apply_filters( "{$this->id_base}_menu_items_reveal_field_args", array(
			'name'    => 'menu_items_reveal' ,
			'label'   => esc_html__( 'Menu items reveal', 'grimlock-animate' ),
			'choices' => array(
	...
	public function change_component_args( $component_args, $attributes ) {
		...
		return array_merge( $component_args, array(
			'menu_items_reveal'  => $attributes['menu_items_reveal'],
	...
	public function sanitize_attributes( $new_attributes ) {
		...
		$attributes['menu_items_reveal']  = isset( $new_attributes['menu_items_reveal'] ) ? sanitize_text_field( $new_attributes['menu_items_reveal'] ) : '';
...
return new Grimlock_Animate_Nav_Menu_Section_Block();
