<?php
/**
 * Grimlock_Animate_Term_Query_Section_Widget_Fields Class
 *
 * @author  Themosaurus
 * @since   1.0.0
 * @package  grimlock
 ...
 * @see WP_Widget
 */
class Grimlock_Animate_Term_Query_Section_Widget_Fields extends Grimlock_Base_Widget_Fields {
	/**
	...
	public function __construct( $id_base = 'grimlock_term_query_section_widget' ) {
		...
		// Animation tab
		add_action( "{$this->id_base}_animation_tab", array( $this, 'add_terms_reveal_field'  ), 100, 2 );
	 ...
	 */
	public function add_terms_reveal_field( $instance, $widget ) {
		$args = array(
			'id'      => $widget->get_field_id( 'terms_reveal' ),
			'name'    => $widget->get_field_name( 'terms_reveal' ),
			'label'   => esc_html__( 'Terms reveal:', 'grimlock-animate' ),
			'value'   => $instance['terms_reveal'],
			'choices' => array(
		...
		$this->select( apply_filters( "{$this->id_base}_terms_reveal_field_args", $args, $instance ) );
	}
	...
	public function change_component_args( $component_args, $instance, $widget_args, $widget_id ) {
		return array_merge( $component_args, array(
			'terms_reveal'  => $instance['terms_reveal'],
	...
	public function sanitize_instance( $new_instance, $old_instance ) {
		...
		$instance['terms_reveal']  = isset( $new_instance['terms_reveal'] ) ? sanitize_text_field( $new_instance['terms_reveal'] ) : '';
...
return new Grimlock_Animate_Term_Query_Section_Widget_Fields();
