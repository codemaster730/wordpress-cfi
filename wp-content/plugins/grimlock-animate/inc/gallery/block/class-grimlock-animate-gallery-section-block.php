<?php
/**
 * Grimlock_Animate_Gallery_Section_Block Class
 *
 * @author  Themosaurus
 ...
 */
class Grimlock_Animate_Gallery_Section_Block {
	/**
	...
	/**
	 * Grimlock_Animate_Gallery_Section_Block constructor.
	 *
	 * @param string $id_base ID of the extended block
	 */
	public function __construct( $id_base = 'grimlock_gallery_gallery_section_block' ) {
		$this->id_base = $id_base;

		add_filter( "{$this->id_base}_panels", array( $this, 'change_panels' ), 10, 1 );

		// Animation tab
		add_filter( "{$this->id_base}_animation_panel_fields", array( $this, 'add_gallery_items_reveal_field'  ), 100 );
		add_filter( "{$this->id_base}_animation_panel_fields", array( $this, 'add_content_reveal_field'        ), 110 );
		add_filter( "{$this->id_base}_animation_panel_fields", array( $this, 'add_reveal_reset_field'          ), 120 );
		add_filter( "{$this->id_base}_animation_panel_fields", array( $this, 'add_reveal_mobile_field'         ), 130 );
	...
	/**
	 * Add a select to set the type of reveal used on the gallery items of the section
	 *
	 * @param array $fields Array of block fields
	 *
	 * @return array Modified array of block fields
	 */
	public function add_gallery_items_reveal_field( $fields ) {
		$fields[] = Grimlock_Base_Block::select_field( apply_filters( "{$this->id_base}_gallery_items_reveal_field_args", array(
			'name'    => 'gallery_items_reveal' ,
	...
	public function change_component_args( $component_args, $attributes ) {
		...
		return array_merge( $component_args, array(
			'gallery_items_reveal' => $attributes['gallery_items_reveal'],
	...
	public function sanitize_attributes( $new_attributes ) {
		...
		$attributes['gallery_items_reveal'] = isset( $new_attributes['gallery_items_reveal'] ) ? sanitize_text_field( $new_attributes['gallery_items_reveal'] ) : '';
...
return new Grimlock_Animate_Gallery_Section_Block();
