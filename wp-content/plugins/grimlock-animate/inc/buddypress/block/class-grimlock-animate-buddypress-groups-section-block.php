<?php
/**
 * Grimlock_Animate_BuddyPress_Groups_Section_Block Class
 *
 * @author  Themosaurus
...
/**
 * The class that extends the Groups Section bloc to add animation options
 */
class Grimlock_Animate_BuddyPress_Groups_Section_Block {
	/**
	...
	/**
	 * Grimlock_Animate_BuddyPress_Groups_Section_Block constructor.
	 *
	 * @param string $id_base ID of the extended block
	 */
	public function __construct( $id_base = 'grimlock_buddypress_groups_section_block' ) {
		$this->id_base = $id_base;

		add_filter( "{$this->id_base}_panels", array( $this, 'change_panels' ), 10, 1 );

		// Animation tab
		add_filter( "{$this->id_base}_animation_panel_fields", array( $this, 'add_groups_reveal_field'   ), 100 );
	 ...
	 */
	public function add_groups_reveal_field( $fields ) {
		$fields[] = Grimlock_Base_Block::select_field( apply_filters( "{$this->id_base}_groups_reveal_field_args", array(
			'name'    => 'groups_reveal' ,
	...
	public function change_component_args( $component_args, $attributes ) {
		...
		return array_merge( $component_args, array(
			'groups_reveal'   => $attributes['groups_reveal'],
	...
	public function sanitize_attributes( $new_attributes ) {
		...
		$attributes['groups_reveal']   = isset( $new_attributes['groups_reveal'] ) ? sanitize_text_field( $new_attributes['groups_reveal'] ) : '';
...
return new Grimlock_Animate_BuddyPress_Groups_Section_Block();
