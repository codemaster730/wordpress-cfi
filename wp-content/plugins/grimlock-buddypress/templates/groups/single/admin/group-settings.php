...
<fieldset class="group-create-privacy mb-3">
...
$group_types_args = array(
	'show_in_create_screen' => true,
);

$group_types = bp_groups_get_group_types( $group_types_args, 'objects' );

if ( ! empty( $group_types ) ) : ?>

	<fieldset class="group-create-types mb-3">
				...
				<?php foreach ( $group_types as $type ) : ?>
					<div class="col-auto py-2">
						<div class="custom-control custom-checkbox m-0">
							<input class="custom-control-input" type="checkbox" name="group-types[]" id="<?php echo esc_attr( sprintf( 'group-type-%s', $type->name ) ); ?>" value="<?php echo esc_attr( $type->name ); ?>" <?php checked( bp_groups_has_group_type( bp_get_current_group_id(), $type->name ) ); ?>/>
							<label class="custom-control-label" for="<?php echo esc_attr( sprintf( 'group-type-%s', $type->name ) ); ?>">
...
<?php if ( bp_is_active( 'groups', 'invitations' ) ): ?>
	<fieldset class="group-create-invitations">
		...
		<div class="radio">
			...
			<label for="group-invite-status-members" class="mb-2"><input type="radio" name="group-invite-status" id="group-invite-status-members" value="members"<?php bp_group_show_invite_status_setting( 'members' ); ?> /> <?php esc_html_e( 'All group members', 'buddypress' ); ?></label>

			<label for="group-invite-status-mods" class="mb-2"><input type="radio" name="group-invite-status" id="group-invite-status-mods" value="mods"<?php bp_group_show_invite_status_setting( 'mods' ); ?> /> <?php esc_html_e( 'Group admins and mods only', 'buddypress' ); ?></label>

			<label for="group-invite-status-admins" class="mb-2"><input type="radio" name="group-invite-status" id="group-invite-status-admins" value="admins"<?php bp_group_show_invite_status_setting( 'admins' ); ?> /> <?php esc_html_e( 'Group admins only', 'buddypress' ); ?></label>
