			...
			<div class="item-body card card-static p-4" id="group-create-body">
				...
				<?php if ( bp_is_group_creation_step( 'group-settings' ) ) : ?>
					...
					<?php if ( $group_types = bp_groups_get_group_types( array( 'show_in_create_screen' => true ), 'objects' ) ) : ?>
						...
						<fieldset class="group-create-types mt-3">
									...
									<?php foreach ( $group_types as $type ) : ?>
										<div class="col-auto py-2">
											<div class="custom-control custom-checkbox m-0">
												<input class="custom-control-input" type="checkbox" name="group-types[]" id="<?php printf( 'group-type-%s', esc_attr( $type->name ) ); ?>" value="<?php echo esc_attr( $type->name ); ?>" <?php checked( true, ! empty( $type->create_screen_checked ) ); ?> />
												<label class="custom-control-label" for="<?php printf( 'group-type-%s', esc_attr( $type->name ) ); ?>">
