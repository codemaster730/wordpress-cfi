...
(function ($) {
    ...
    $.fn.customizerTabs = function (options) {
        ...
        $.each(options, function (i, tab) {
            ...
            $.each(tab.controls, function (j, controlName) {
				...
				fontFields.forEach( function( field ) {
					control = wp.customize.control(controlName + '[' + field + ']');
					if (control)
						control.container.addClass(tab.class);
				} );
            });
    ...
    wp.customize.bind('pane-contents-reflowed', function () {
        ...
        if (!initializedTabs) {
            ...
            $('.grimlock-customizer-tabs a').click(function(e) {
                ...
                $(tabSelector).siblings('li:not(.customize-section-description-container)').addClass( 'grimlock-customizer-tab-control--hidden' );
                $(tabSelector).each(function () {
                	var controlName = this.id.replace('customize-control-', '');
                    var control = wp.customize.control(controlName);

                    // Only show the control if it is currently active to prevent conflict with active callbacks
                    if (control && control.active())
						control.container.removeClass( 'grimlock-customizer-tab-control--hidden' );
	...
	/**
	 * Open the correct tab when clicking a floating pen button in the customizer preview
	 */
    wp.customize.bind( 'ready', function() {
		wp.customize.previewer.bind( 'focus-control-for-setting', function( settingId ) {
			...
			wp.customize.control( settingId, function( control ) {
				...
				$tabs.each( function() {
					...
					if ( control.container.hasClass( tabClass ) ) {
						// Open the tab
						$( this ).click();
					}
				} );
