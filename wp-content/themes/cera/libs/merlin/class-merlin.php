 ...
 * @author    Rich Tabor, from ThemeBeans.com & the team at AwesomeMotive.com
...
class Merlin {
	...
	public function admin_page() {
		...
		// Persist the "demo" query arg through ajax requests to make sure modifications depending on that query arg will work throughout the whole installation process
		$ajaxurl = $this->maybe_add_query_args( admin_url( 'admin-ajax.php' ) );
	...
	public function steps() {
		...
		// Show the demo selector, only if there's demo content added.
		if ( ! empty( $this->import_files ) ) {
			$this->steps['demo'] = array(
				'name' => esc_html__( 'Demo', 'merlin-wp' ),
		...
		if ( ! empty( $this->import_files ) && isset( $_GET['demo'] ) ) {
	...
	protected function step_next_link() {
		...
		return add_query_arg( 'step', $keys[ $step ], $prefix . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'] );
	}
	...
	protected function demo() {
		...
		// Text strings.
		$header    = $strings['demo-header'];
		$paragraph = $strings['demo'];
		$skip      = $strings['btn-skip'];
		$next      = $strings['btn-next'];

		$import_files_indexes = array_keys( $this->import_files );
		$first_import_file_index = array_shift( $import_files_indexes );
		...
		<div class="merlin__content--transition">
			...
			<?php foreach ( $this->import_files as $index => $import_file ) : ?>
				<?php if ( ! empty( $import_file['preview_url'] ) ) : ?>
					<a href="<?php echo esc_url( $import_file['preview_url'] ); ?>" target="_blank">
				<?php endif; ?>

				<?php if ( ! empty( $import_file['import_file_screenshot'] ) ) : ?>
					<img style="display: none; max-width: 100%; padding-bottom: 2.8em;" src="<?php echo esc_url( $import_file['import_file_screenshot'] ); ?>" class="import-file-screenshot import-file-screenshot-<?php echo esc_attr( $index ); ?>" />
				<?php endif; ?>
...
<!--				<div class="merlin__demo-import-use-elementor-control">-->
<!--					<img src="--><?php //echo trailingslashit( $this->base_url ) . $this->directory . '/assets/images/elementor.png'; ?><!--" alt="Elementor logo">-->
<!--					<label for="merlin-demo-import-use-elementor"><strong>--><?php //esc_html_e( 'I want to use Elementor', 'merlin-wp' ); ?><!--</strong></label>-->
<!--					<input id="merlin-demo-import-use-elementor" type="checkbox" class="merlin__input js-merlin-demo-import-use-elementor" />-->
<!--				</div>-->

				<br/>

				<?php foreach ( $this->import_files as $index => $import_file ) : ?>
					<?php if ( ! empty( $import_file['import_file_warning'] ) ) : ?>
						<div style="display: none;" class="alert alert-warning import-file-warning import-file-warning-<?php echo esc_attr( $index ); ?>">
							<?php echo wp_kses( $import_file['import_file_warning'], $allowed_html_array ); ?>
						</div>
					<?php endif; ?>
				<?php endforeach; ?>
		...
		<form action="" method="post" class="<?php echo esc_attr( $multi_import ); ?>">
			...
			<footer class="merlin__content__footer">
				...
				<a href="<?php echo esc_url( $this->maybe_add_query_args( $this->step_next_link(), $first_import_file_index ) ); ?>" class="merlin__button merlin__button--next">
		...
		<?php
		$this->logger->debug( __( 'The demo select step has been displayed', 'merlin-wp' ) );
	}

	/**
	 * Add query args to the given url to persist it through different requests
	 *
	 * @param string $url The url to alter
	 * @param bool $demo_index If not set, will use the value of $_GET['demo']
	 *
	 * @return string
	 */
	protected function maybe_add_query_args( $url, $demo_index = false ) {
		// Don't use empty() cause these can be equal to 0
		if ( $demo_index === false && ! isset( $_GET['demo'] ) ) {
			return $url;
		}

		if ( $demo_index === false && isset( $_GET['demo'] ) ) {
			$demo_index = $_GET['demo'];
		}

		$query_args = array(
			'activate-multi' => 1,
			'demo'           => $demo_index,
			'use-elementor'  => ! empty( $_GET['use-elementor'] ) ? 1 : 0,
		);
	...
	protected function ready() {
		...
		<div class="merlin__content--transition">
			...
			<p><?php echo wp_kses( sprintf( $paragraph, $author ), $allowed_html_array ); ?></p>
				...
				<?php if ( ! empty( $_GET['demo'] ) && ! empty( $this->import_files[ $_GET['demo'] ]['import_finished_warning'] ) ) : ?>
					<div style="margin-top: 1rem;" class="alert alert-warning">
						<?php echo wp_kses( $this->import_files[ $_GET['demo'] ]['import_finished_warning'], $allowed_html_array ); ?>
					</div>
				<?php endif; ?>

		</div>

		<footer class="merlin__drawer--open merlin__content__footer merlin__content__footer--fullwidth <?php echo esc_attr( $links_class ); ?>">
	...
	function _ajax_plugins() {
		...
		// Persist the "demo" query arg through ajax requests to make sure modifications depending on that query arg will work throughout the whole installation process
		$tgmpa_url = $this->maybe_add_query_args( $this->tgmpa->get_tgmpa_url() );
	...
	function _ajax_content() {
		...
		if ( isset( $_POST['proceed'] ) ) {
			...
			$json = array(
				'url'            =>  $this->maybe_add_query_args( admin_url( 'admin-ajax.php' ) ),
	...
	public function pt_importer_new_ajax_request_response_data( $data ) {
		$data['url']      = $this->maybe_add_query_args( admin_url( 'admin-ajax.php' ) );
