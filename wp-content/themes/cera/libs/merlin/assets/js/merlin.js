...
var Merlin = (function($){
    ...
    function window_loaded(){
        ...
        var changeButtonUrl = function( key, value ) {
            var buttonNext = $( 'a.merlin__button--next' );

            var nextUrl = new URL( buttonNext.attr( 'href' ) );
            var nextUrlParams = new URLSearchParams( nextUrl.search );
            nextUrlParams.set( key, value );
            nextUrl.search = nextUrlParams.toString();

            buttonNext.attr( 'href', nextUrl.href );
        };

        $( document ).on( 'change', '.js-merlin-demo-import-select', function() {
            var selectedIndex = $(this).val();

            changeButtonUrl( 'demo', selectedIndex );
        } );

        $( document ).on( 'change', '.js-merlin-demo-import-use-elementor', function() {
            changeButtonUrl( 'use-elementor', $(this).is(':checked') ? 1 : 0 );
        } );

        var $importFileSelect = $( '.js-merlin-demo-import-select' );
        var $importFilesScreenshots = $( '.import-file-screenshot' );
        var $demoStepDefaultIcon = $( '.icon.icon--content' );
        var $importFilesWarnings = $( '.import-file-warning' );

        var showImportFileScreenshot = function() {
            $importFilesScreenshots.hide();
            $demoStepDefaultIcon.hide();

            var $importFileScreenshot = $( '.import-file-screenshot-' + $importFileSelect.val() );

            if ( $importFileScreenshot.length ) {
                $importFileScreenshot.show();
            }
            else {
                $demoStepDefaultIcon.show();
            }
        };

        var showImportFileWarning = function() {
            $importFilesWarnings.hide();
            $( '.import-file-warning-' + $importFileSelect.val() ).show();
        };

        showImportFileScreenshot();
        showImportFileWarning();

        $importFileSelect.change( function() {
            showImportFileScreenshot();
            showImportFileWarning();
        } );
    ...
    function ContentManager(){
        ...
        var selected_demo = current_url.searchParams.get('demo') || 0;
                ...
                if(typeof response.url !== "undefined"){
                    ...
                    if(response.hash === current_item_hash){
                        ...
                        if ( typeof response.selected_index === "undefined" ) {
                            response.selected_index = selected_demo;
                        }
